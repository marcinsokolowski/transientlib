...
char	visibility_array[SAT_NUMBER], tracking_mode[30];

float	az_array[SAT_NUMBER], el_array[SAT_NUMBER], long_array[SAT_NUMBER], lat_array[SAT_NUMBER],
	footprint_array[SAT_NUMBER], range_array[SAT_NUMBER], altitude_array[SAT_NUMBER],
	velocity_array[SAT_NUMBER], eclipse_depth_array[SAT_NUMBER], phase_array[SAT_NUMBER],
	squint_array[SAT_NUMBER];

double	doppler[SAT_NUMBER], nextevent[SAT_NUMBER];

long	aos_array[SAT_NUMBER], orbitnum_array[SAT_NUMBER];
...
char ReadDataFiles()
{
	...
	if (fd!=NULL)
	{
		while (x<SAT_NUMBER && feof(fd)==0)
		{
			/* Initialize variables */
			/*if( (x>0 && x%1000==0) || x>=5060 )
				printf("x=%d, ",x);*/
		...
		if( gSatlibTraceLevel>0 )printf("read %d satelites from TLE file\n",x);
		if( gSatlibTraceLevel>0 )printf("Current sat DB limit is : %d\n",SAT_NUMBER);	
...
double GetDaynum( time_t ut_time )
{
	/* Read the system clock and return the number
	   of days since 31Dec79 00:00:00 UTC (daynum 0) */
	...
	return ( ( ((double)ut_time)/86400.0 ) - 3651.0);
}
...
void MultiTrack()
{
	...
	unsigned char	satindex[SAT_NUMBER], inrange[SAT_NUMBER], sunstat=0, ok2predict[SAT_NUMBER];

	double		aos[SAT_NUMBER], aos2[SAT_NUMBER], temptime,
			nextcalctime=0.0, los[SAT_NUMBER], aoslos[SAT_NUMBER];
...
int InitSatLib( const char* qth_file, const char* tle_file, const char* db_file )
{
	...
	/*   y=argc-1; */
		...
		if(!qth_file || strlen(qth_file)==0){
			strcpy(qthfile,"predict.qth");
		}else{
			strcpy(qthfile,qth_file);
		}
		if(!tle_file || strlen(tle_file)==0){
			strcpy(tlefile,"predict.tle");
		}else{
		   strcpy(tlefile,tle_file);
		}	   	
		if(!db_file || strlen(db_file)==0){
	   	strcpy(dbfile,"predict.db");
		}else{
			strcpy(dbfile,db_file);
		}   	
...
int CalcCurrentPosition( int x )
{
	...
	printf("PREDICT: Tracking %-10s\007",sat[x].name); 
...
int CalcCurrentPositionOfSingle( int x, time_t ut_time, struct satInfo* sat_info )
{
	/* This function tracks a single satellite in real-time
	   until 'Q' or ESC is pressed.  x represents the index
	   of the satellite being tracked.  If speak=='T', then
...
/*		calculation of LOS time and next visiblity time skipped here :
		if (sat_ele>=0.0 && geostationary==0 && decayed==0 && daynum>lostime)
		{
			lostime=FindLOS2();
			aoslos=lostime;
		...
		strcpy(sat_info->sat_name,sat[x].name);
		strcpy(sat_info->sat_code,sat[x].designator);
		sat_info->visibility=visibility_array[indx];
		sat_info->sat_azim=sat_azi;
		sat_info->sat_ele=sat_ele;
		sat_info->sat_alt=sat_alt;
		sat_info->sat_geo_stat=geostationary;
...
void CheckAll( time_t ut_time )
{
	...
	printf("SatName Visible Azim Ele Altitude\n");
	for(x=0;x<gSatNumber;x++){
		struct satInfo sat_info;
		if(CalcCurrentPositionOfSingle( x, ut_time, &sat_info )){
			printf("%s\t%c\t%.2f\t%.2f\t%.2f\n",sat_info.sat_name,
						sat_info.visibility, sat_info.sat_azim, sat_info.sat_ele,
						sat_info.sat_alt );
		}
	}		
}