...
/****************************************
** read compressed  fits data
****************************************/
int read_compressed_fits_data(fd,nx,ny,bp,bs,bz,data,blocksize,divisor)
int fd,nx,ny,bp;
void **data;
float bs,bz;
int blocksize, divisor;
{
  ...
  if( (*data = (void *)malloc(nx*ny*bpp)) == NULL)return(-1);

  if ( (obuf = (char *)malloc(4*nx)) == NULL){
    free(*data);
    return(-1);
  }
  ...
  for(j=0; j<ny*nz; j++){
    ...
    left=fits_rdecomp(obuf,n,idata,nx,blocksize,2);
    if(left)memcpy(obuf, obuf+n-left, left);
    dat = convert_data(idata,nx,32,bs,bz,bp);
//    write_fits_data(fdo,dat,nx,1,bp,0);
